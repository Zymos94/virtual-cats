...
**Current: pre-refactor**, single file `src/store/petStore.ts` (Zustand, ~900 lines) — pets, scene
items, tail segments, the draggable UI panel's own position, save/load triggering, time-scale, all
in one store. Its `tick()` is one large synchronous pass, in order:
...
5. For each non-suitcased pet: age it, then either continue petting or score every item/other cat
   via `attentionScore()` (`src/game/attention.ts`), feed the winner into the pure FSM
   (`src/game/behaviorFSM.ts`), then move it (`src/game/movement.ts`).
6. A second pass resolves mutual cat-cat arrival and item consumption (needs the _other_ entity's
   already-updated state from this same tick).
7. Step tail-chain physics per pet (`src/game/tailPhysics.ts`, anchor from `tailMood.ts`).
...
and route any new `Pet`/save field through it rather than adding another ad-hoc default.
...
- `src/game/useGameLoop.ts` / `useSceneBounds.ts` / `useDraggable.ts` / `mousePosition.ts` — the
  RAF loop, viewport sync, generic click-vs-drag hook, mutable mouse-position singleton.
...
- `src/components/Scene.tsx` — pure render: items then pets, in that z-order.
...
`itemPhysics.ts`, `genetics.ts`, `behaviorFSM.ts`. Run `npm test` and `npm run typecheck` before
every commit (the pre-commit hook does this automatically). New pure `src/game/*` modules should
get a sibling `*.test.ts`.
...
- The current cat gait has a known moonwalk artifact (legs cycle out of phase with real footfall)
  — fix tracked as `ANIMATION_PLAN.md` Phase A1, first priority of that plan.
